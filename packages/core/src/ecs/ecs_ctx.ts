...
> {
  ...
  >(
    entId: string,
    compoType: TCompoType,
    compo_factory: (entId: string) => TCompoMetaMap[TCompoType]["compo"]
  ) {
    let compo: any = this.get_compo(entId, compoType);
    if (!compo) {
      compo = compo_factory(entId) as any;
